defmodule OpenSentience.Application do
  ...
  defp with_optional(children, cfg_key, module, arg) do
    enabled? = enabled_in_config?(cfg_key)
  ...
  defp enabled_in_config?(cfg_key) do
    # Important: many of our configs are keyword lists (e.g. `launcher: [agents_dir: ..., logs_dir: ...]`).
    # Treat a keyword-list config as "enabled by default" unless it explicitly sets `enabled: false`.
    value = Application.get_env(:opensentience_core, cfg_key, :__missing__)
