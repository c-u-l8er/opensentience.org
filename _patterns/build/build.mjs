...
${conclusionFilm ? `<p>Reduced whole by TRVM's forge in ${conclusionFilm.execution_identity.seconds}s on ${esc(conclusionFilm.execution_identity.host)} — ${conclusionFilm.epochs.length} epochs, ${new Set(conclusionFilm.epochs.map((e) => e.film_hash)).size} distinct film hashes; the forge's id equals the seal above.</p><div class="sf" id="board"><div class="sf-stage">${SF.svg(SF.computeState(boardScene(conclusionFilm, conclusionSeal.graph, 'The board before epoch 1.'), 0), { caption: false })}</div></div>` : `<div class="sf-static">${conclusionSeal && conclusionSeal.ok ? boardStatic(conclusionSeal.graph) : ''}</div><p class="warn">The whole-board film has not been reduced yet (<code>run-films.mjs conclusion</code>); the board above is the sealed topology.</p>`}
