...
@media(max-width:900px){.book{grid-template-columns:minmax(0,1fr)}.side{position:static;max-height:none;border-right:0;border-bottom:1px solid var(--line,#e7e0d2);padding-bottom:.8rem}.two{grid-template-columns:1fr}dl.honest{grid-template-columns:minmax(0,1fr)}dl.honest dt{padding-top:.5rem}main table{display:block;max-width:100%;overflow-x:auto}code,.semid,.sf-insp dd{overflow-wrap:anywhere}}
` + SF.CSS;
...
/* the keys of a Film line, per role, in the order a reader wants them: what the object IS now, then
   what it is wired to, then the fixed configuration it was sealed with. Anything the forge emits that
   is not listed still appears, after these — the readout never silently drops a field. */
const FACT_ORDER = { spinner: ['rotor', 'socket', 'config', 'policy', 'w', 'n'], orb: ['pose', 'fault', 'controller', 'policy', 'w', 'n'], pulser: ['armed', 'done', 'nf', 'mode', 'p', 'phase'], door: ['open', 'next_open'], relay: ['cur_out', 'next_out'] };
/* A Film line also carries bare flags with no '=' — `quat4` is one. They are not in kv, which is
   built from key=value pairs, and a readout that quietly dropped them would be claiming to show the
   whole line while showing less. Hex quads (the members of a rotor= or pose= tuple) are not flags. */
const flagsOf = (f) => ((f.raw.split(':')[2] || '').split(',').filter((t) => t && !t.includes('=') && !/^[0-9a-f]{1,8}$/.test(t)));
const factsOf = (f) => { const order = FACT_ORDER[f.role] || []; const keys = [...order.filter((k) => f.kv[k] !== undefined), ...Object.keys(f.kv).filter((k) => !order.includes(k))]; return [...keys.map((k) => [k, f.kv[k]]), ...flagsOf(f).map((t) => [t, 'set'])]; };
function filmSceneFrom(rc, g, intro) {
  ...
  rc.epochs.forEach((ep, i) => {
    ...
    for (const l of ep.film) {
      ...
      /* the line is restated only in the epoch it changes; between changes it stands, which is what
         the forge means by it. The state action runs every epoch — it is what unlights the object. */
      if (changed || !prev[f.name]) actions.push({ op: 'note', target: f.name, text: note }, { op: 'facts', target: f.name, pairs: factsOf(f) });
      actions.push({ op: 'state', target: f.name, state: changed ? 'admitted' : (f.role === 'orb' && f.kv.fault === '1' ? 'refused' : 'idle'), changed });
    }
    if (hasLedger) {
