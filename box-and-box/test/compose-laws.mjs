...
import { Brick, ZERO, isZero, none, idBrick, composeAnd, composePipe, composeTree,
         ...
         createComposeRuntime, isAttested, adopt, ingestFrame, ingestJSON, canonBytes,
...
const CERTS = [
  ...
  ['CERT34', 'the hostile-data boundary is TOTAL: ingest yields a named 0-bar or a Brick, never an exception', (n) => trial(n, () => {
    ...
    let out;
    try { out = adopt(v); } catch (e) { return `adopt threw ${e.constructor.name} on: ${label}`; }
  ...
  ['CERT37', 'descriptor-only ingress really is descriptor-only: no caller code runs during a snapshot', (n) => trial(n, () => {
    ...
    let out;
    try { out = rt.adopt(raw); } catch (e) { return `adopt threw ${e.constructor.name}`; }
  ...
  ['CERT38', 'the state WORLD requires is reachable by ONE call: ingested AND authenticated AND immutable', (n) => trial(n, () => {
    ...
    for (const [label, bad] of [
      ...
      let r;
      try { r = rt.adoptAndVerify(bad); } catch (e) { return `adoptAndVerify threw ${e.constructor.name} on ${label}`; }
  ...
  ['CERT39', 'the hostile boundary takes BYTES, because reflection over an object is executable', (n) => trial(n, () => {
    ...
    let viaObj;
    try { viaObj = rt.adoptAndVerify(spy); } catch (e) { return `adoptAndVerify threw ${e.constructor.name}`; }
  ...
  ['CERT40', 'an own `undefined` is not absence: the boundary never erases present evidence', (n) => trial(n, () => {
    ...
    for (const [label, out] of [['adopt', adopt(raw)], ['adoptAndVerify', rt.adoptAndVerify(raw)]]) {
...
const WIRE = [
  ['WIRE0', 'the WORLD door is the same function however the caller got hold of it', (n) => trial(n, () => {
    ...
    // Every ingress name, fetched every way a caller fetches one.
    for (const name of ['ingestFrame', 'ingestJSON', 'adopt', 'ingestFrameAndVerify', 'ingestJSONAndVerify', 'adoptAndVerify']) {
      const bound = rt[name];                                    // detached — no receiver
      let direct, detached, viaMap;
      try { direct = rt[name](arg); } catch (e) { return `rt.${name}() threw ${e.constructor.name}`; }
      try { detached = bound(arg); } catch (e) { return `const {${name}} = rt threw ${e.constructor.name} — the door depends on how it was fetched`; }
      try { viaMap = [arg].map(rt[name])[0]; } catch (e) { return `[x].map(rt.${name}) threw ${e.constructor.name}`; }
